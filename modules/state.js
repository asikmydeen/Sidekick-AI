...
export let state = {
  ...
  providerCredentials: {
    ...
    ollama: { endpoint: 'http://127.0.0.1:11434', selectedModel: '', models: [] },
    bedrock: {
      accessKey: '',
      secretKey: '',
      sessionToken: '',
  ...
  if (stored.chatState) {
    ...
    if (!stored.chatState.providerCredentials) {
      ...
      if (stored.chatState.endpoint) {
        stored.chatState.providerCredentials.ollama.endpoint = stored.chatState.endpoint || 'http://127.0.0.1:11434';
      }
    ...
    if (!state.providerCredentials.ollama.endpoint) {
      state.providerCredentials.ollama.endpoint = 'http://127.0.0.1:11434';
    }
